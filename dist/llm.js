...
exports.analyzeLearningNeeds = analyzeLearningNeeds;
...
// 分析学习需求的深层痛点
async function analyzeLearningNeeds(userInput) {
    const prompt = `
...
// 生成 MCP 服务代码
async function generateMCPCode(serviceType, keywords, need) {
    // 如果有深度分析结果，使用更详细的提示词
    if (need && need.mcp_tools && need.mcp_tools.length > 0) {
        const tools = need.mcp_tools.map((t) => typeof t === 'string' ? t : `${t.name}: ${t.description}`).slice(0, 3); // 选择前3个最重要的工具
        const prompt = `
