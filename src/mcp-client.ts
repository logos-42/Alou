...
export async function callMCPCompass(query: string, maxRetries: number = 3): Promise<any> {
    ...
    try {
      ...
      client = await Promise.race([
        ...
        new Promise<never>((_, reject) => 
          setTimeout(() => reject(new Error('MCP 客户端连接超时')), 5000)
        )
      ]);
      ...
      // 查找推荐服务的工具
      const recommendTool = tools.tools.find(t => 
        t.name.includes('recommend') || t.name.includes('search')
      );
      ...
      if (recommendTool) {
        ...
        const result = await Promise.race([
          ...
          new Promise<never>((_, reject) => 
            setTimeout(() => reject(new Error('MCP Compass API 调用超时')), 15000)
          )
        ]);
...
export async function callMCPInstaller(packageName: string): Promise<any> {
  ...
  try {
    ...
    // 查找安装工具
    const installTool = tools.tools.find(t => 
      t.name.includes('install') && t.name.includes('repo')
    );
...
export async function callMCPCreate(language: string, code: string, serviceType?: string): Promise<any> {
  ...
  for (let i = 0; i < configs.length; i++) {
    ...
    try {
      ...
      client = await Promise.race([
        createMCPClient(config.command, config.args, config.env),
        new Promise<never>((_, reject) => 
          setTimeout(() => reject(new Error('MCP 客户端连接超时')), 10000)
        )
      ]);
      ...
      if (config.args?.[0]?.includes('MCP-Server-Creator')) {
        ...
        const createTool = tools.tools.find(t => 
          t.name.includes('create') && (t.name.includes('server') || t.name.includes('mcp'))
        );
        ...
        if (createTool) {
          ...
          const result = await Promise.race([
            ...
            new Promise<never>((_, reject) => 
              setTimeout(() => reject(new Error('MCP API 调用超时')), 30000)
            )
          ]);
